...
export default function UserManagementTable() {
  ...
  const handleOpenUpdateDialog = (user: User) => {
    ...
    setNewFaceUri(null); // Reset previously captured face
    setIsDialogOpen(true);
  };

  const handleFaceCapturedInDialog = (dataUrl: string) => {
    setNewFaceUri(dataUrl);
  };

  const handleUpdateUserFace = async () => {
    if (!selectedUser || !newFaceUri) return;

    setIsUpdating(true);
    try {
      const success = await updateUserFaceAdmin(selectedUser.id, newFaceUri);
      if (success) {
        toast({ title: "Face Updated", description: `${selectedUser.name}'s login face has been updated.` });
        setIsDialogOpen(false); // Close dialog on success
      } else {
        toast({ title: "Update Failed", description: "Could not update the face. Please try again.", variant: "destructive" });
      }
    } catch (error) {
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Capture a new face image. This will replace the existing login face.
              </DialogDescription>
            </DialogHeader>
            <div className="py-4 space-y-4">
              <div className="flex flex-col items-center">
                ...
                <FaceCapture onFaceCaptured={handleFaceCapturedInDialog} imageSize={200} />
                {newFaceUri && <p className="text-xs text-green-600 mt-2">New face captured. Ready for update.</p>}
              </div>
            </div>
            <DialogFooter>
              <DialogClose asChild>
              ...
              <Button type="button" onClick={handleUpdateUserFace} disabled={isUpdating || !newFaceUri}>
...
// Minimal Card component for structure, assuming shadcn/ui Card is available elsewhere or this is a simplified version
...
// Minimal cn utility if not globally available (it should be via lib/utils)
