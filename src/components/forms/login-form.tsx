...
export default function LoginForm() {
  ...
  const { loginWithFace, users, loading: authLoading } = useAuth(); // Access users and loading state
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    // This check is primarily for UI feedback; actual user existence is handled by loginWithFace.
    ...
    try {
      ...
      if (success) {
        ...
        // Toast for login failure (e.g., face not processed, no users) is handled within loginWithFace
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Button type="submit" disabled={isLoggingIn || !capturedFaceUri || (!authLoading && users.length === 0)} className="w-full">
        {isLoggingIn ? (
